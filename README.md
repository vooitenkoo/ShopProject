...
Исходный код и `docker-compose` лежат в каталоге [`Shop/`](Shop/).
...
- xUnit — тесты в `Shop/UserManagmentService/test`
...
> **Замечание:** в `Shop/ApiGateway/ocelot.json` для downstream указаны `localhost` и порты сервисов. Для сценария «только Docker» может потребоваться замена хостов на имена сервисов из `docker-compose` (`user-service`, `product-service`) и схемы `http`, если шлюз не достучится до бэкендов.
...
```bash
cd Shop/UserManagmentService/src/WebApi
dotnet run
```

```bash
cd Shop/ProductManagmentService/src/WebApi
dotnet run
```
...
```bash
cd Shop/UserManagmentService/test
dotnet test
```

```bash
cd Shop/ProductManagmentService/test/ProductManagmentServiceTests
dotnet test
```
